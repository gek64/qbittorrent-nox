...
install_openssl() {
  ...
  git clone --depth 1 --branch $branch $openssl
  cd openssl || exit
  ./Configure threads no-shared no-dso no-comp \
              CXXFLAGS="-static" CPPFLAGS="-static" LDFLAGS="-static" \
              linux-generic64
  make -j "$(nproc)"
  make install_sw
  cd ..
...
#linux64-s390x linux64-sparcv9 mingw mingw64 nextstep nextstep3.3 purify
